...
// Thân phiếu tăng ca A4 (article) — TÁCH từ /dashboard/registrations/[id]/view
// để dùng chung 2 nơi: trang xem/in 1 phiếu + trang in gộp cả ngày (print-day).
// Layout/print CSS đã nghiệm thu bản giấy — sửa gì ở đây phải soi lại cả 2 trang.
// Chiều cao dòng per-phiếu truyền qua CSS var --ot-row-h (mỗi phiếu 1 giá trị
// riêng, vì trang in gộp có nhiều phiếu số dòng khác nhau trên cùng 1 trang CSS).
...
// CSS in dùng chung — y hệt trang view cũ, chỉ khác height dòng đọc từ
// var(--ot-row-h) do article tự khai (trước đây interpolate thẳng vào chuỗi).
export const OT_PRINT_CSS = `
  ...
  @media print {
    ...
    .ot-form table td {
      ...
      height: var(--ot-row-h, 10mm) !important;
    }
...
export default async function OvertimeSheetArticle({
  regId,
  homeHref = '/dashboard',
}: {
  regId: string;
  ...
  if (!reg) {
    return <p className="text-red-600">Không tìm thấy phiếu.</p>;
  }
  ...
  return (
    <article
      className="ot-form bg-white p-6 md:p-10 shadow-sm print:shadow-none print:p-0 text-[13px] text-black"
      style={{ '--ot-row-h': `${rowHeightMm}mm` } as React.CSSProperties}
    >
